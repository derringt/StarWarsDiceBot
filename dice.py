...
def display_results(results):
    ...
    input: A dictionary of results.
    output: A string of results.
    """
    str_list = []

    if results['success'] < 1:
        str_list.append("The roll failed with {failure} failure.".format(**results))
    else:
        str_list.append("The roll succeeded with {success} success!".format(**results))

    if results['advantage'] > 0:
        str_list.append("The roll generated {advantage} advantage!".format(**results))
    elif results['advantage'] < 0:
        str_list.append("The roll generated {threat} threat!".format(**results))

    if results['despair'] > 0:
        str_list.append("The roll generated {despair} despair..".format(**results))
    if results['triumph'] > 0:
        str_list.append("The roll generated {triumph} triumph!".format(**results))

    if results['light'] > 0 or results['dark'] > 0:
        str_list.append("The roll generated {light} light and {dark} dark force points!"
              .format(**results))
